...
to setup
 clear-all
  set restart false ; command used to restart when a player wins

  ask patches [set countdown 100]


  ifelse netlogo-web? [set max-sheep 10000] [set max-sheep 30000]

; most of set up was taken from sheep wolf predation I added another breed for second wolf, and eleminated many of the user slider controlled variables
...
; only turtles with labels are the two wolves the label displays energy
...
if restart = true  ; condition to restart game when one player wins

  [stop setup]
                  ;sheep breed just eats grass and moves randomly
  ask sheep [
    move
...
; both my breeds of wolves can eat the other wolf breed and they can eat sheep
  ]
  ask wolves [
   ; move
    set energy energy - 1
    eat-sheep
    eat-lama
  ]                 ; lamas are just the other wolf breed represented on screen as blue a wolf
   ask lamas [
   eat-sheep
    set energy energy - 1
    eat-wolf
...
to move            ; move function taken from original wolf sheep predation model
  rt random 50
  lt random 50
  fd 1
end

to eat-grass         ; describes how the sheep eat grass
...
to eat-sheep
 ...
 ; decscribes how wolves eat eachother under condition that energy is greater
  let prey one-of sheep in-radius 3
  if prey != nobody  [
    ask prey [ die ]
   set energy energy + 50    ; get energy from eating
  ]
end
...
to eat-wolf
  ...
  if any? wolves [
  if ([energy] of wolf 0 < [energy] of lama 1) [
;set lives (lives - 1)
      ask wolves in-radius 4[ask wolves [ die ] ]
      if not any? wolves  [ user-message "Blue wolf wins!"  set restart true ]
show restart
 ]
...
to eat-lama
  ...
  if any? lamas [
  if ([energy] of lama 1 < [energy] of wolf 0) [
;set lives (lives + 1)
  ask lamas in-radius 4[ask lamas [ die ] ]
      if not any? lamas  [ user-message "Black wolf wins!"  set restart true ]
...
to display-labels
  ...
  if show-energy? [
    ...
    ask lamas [ set label round energy ]
...
; commands for the movement of wolf breeds

to move-left
  ask wolves with [xcor != min-pxcor]
    [ set heading 270
...
to move-right
  ask wolves with [xcor != max-pxcor]
    [ set heading 90
      fd 2
     set energy energy - 1
    ]
...
to move-up
  ask wolves with [ycor != max-pycor]
    [ set heading 0
      fd 2
    set energy energy - 1
    ]
...
to move-down
  ask wolves with [ycor != min-pycor]
    [ set heading 180
      fd 2
     set energy energy - 1
    ]
